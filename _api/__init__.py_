...
# Ensure these files/routers exist if you're importing them
...
# CORS
# -----
# Set allow_origins to ["*"] for broader debugging of WebSocket 403
# For production, you should revert this to specific origins from settings.CORS_ALLOWED_ORIGINS
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], # <--- CHANGED: Set to "*" for debugging WebSocket 403
...
# Include Routers
# ---------------
# Adjusting prefixes to directly match the observed `/api/facebook/...` calls
# If you intend to use `/api/v1/`, you'll need to re-add `settings.API_V1_STR`
# and ensure your frontend requests use the /v1 prefix.

app.include_router(
    auth_router,
    prefix="/api/facebook/auth", # <--- ADJUSTED prefix
    tags=["Facebook Authentication"],
)

app.include_router(
    status_router,
    prefix="/api/facebook/status", # <--- ADJUSTED prefix to match the 404 log
    tags=["Facebook Status"],
)

app.include_router(
    posts_router,
    prefix="/api/facebook", # <--- ADJUSTED prefix, so it's /api/facebook/posts
    tags=["Facebook Posts"],
)

app.include_router(
    insights_router,
    prefix="/api/facebook/insights", # <--- ADJUSTED prefix
    tags=["Facebook Insights"],
)

app.include_router(
    webhooks_router,
    prefix="/api/facebook/webhooks", # <--- ADJUSTED prefix
    tags=["Facebook Webhooks"],
)

app.include_router(
    bot_router,
    prefix="/api/bot", # <--- ADJUSTED prefix
    tags=["AI Bot"],
)

app.include_router(
    website_router,
    prefix="/api/agents", # <--- ADJUSTED prefix
    tags=["Agent Websites"],
)
